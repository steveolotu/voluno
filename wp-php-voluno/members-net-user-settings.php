...
if($page_section == "main part"){
	...
	if(1==1){
	    ...
	    #Advisor (open)
	    if(1==2 AND in_array('Volunteer Advior',$function_userpositions_get['simple_pure_array']['accepted'])){ ###
	...
	if(1==1){
	    ...
	    $user_and_default_settings_query = $GLOBALS['wpdb']->prepare('SELECT *
	    ...
	    $user_and_default_settings_results = $GLOBALS['wpdb']->get_results($user_and_default_settings_query);
	...
	/*
		...
		elseif($settings_array[$x][0]=="checkbox"){
		    $my_settings_query = $GLOBALS['wpdb']->prepare('SELECT *
						FROM fi4l3fg_voluno_personal_settings
		    ...
		    $my_settings_results = $GLOBALS['wpdb']->get_results($my_settings_query);
