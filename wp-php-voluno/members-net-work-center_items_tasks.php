...
/*
ARM
This page used Access Rights Management (ARM).
The idea is to
a) define the role of the current user
b) to define which user has access to which content
c) to combine the two, selecting the content elements the user may access
...
When a mysql update changes the task status, the permissions have already been defined. Thus, they need to be redifined. To do
...
if(1==1){
	...
	#if new task, make sure task 1 exists (open)
	if(1==1){
	    ...
	    #task id for query definition (open)
	    if(empty($task_id_only_for_query_define_only_once['only_define_in_first_iteration'])){
		#new tasks (id = 1) change the $get_id variable. this disturbs the query.
		#to prevent this, the query id is defined only once, during the first iteration
		$task_id_only_for_query_define_only_once['only_define_in_first_iteration'] = $get_id;
	    }
	    #task id for query definition (close)
	    
	}
	#if new task, make sure taks 1 exists (close)
	
	$task_query = $GLOBALS['wpdb']->prepare('SELECT *
				    ...
				    WHERE task_id = %d;'
				    ,$task_id_only_for_query_define_only_once['only_define_in_first_iteration']);
	...
	if(in_array('NGO Member',$function_userpositions_get['simple_pure_array']['accepted'])){
	    ...
	    #ngo - author - create new task (ngo_select + edit + 1) (open)
	    ...
	    if($task_status == "draft" AND empty($user_role) AND $_GET['edit_task'] == "1"){
		if($task_row->task_author_id == $GLOBALS['voluno_variables']['current_user_extended']->usersext_id){
		    $user_role = "ngo - author - edit";
		}
	    }
	    #ngo - author - edit (close)
	    
	    #ngo - author's ngo - edit (open) #modified
	    if($task_status == "draft" AND empty($user_role)){/*
		if($_GET['edit_task'] == "1"){
		    $user_role = "ngo - author's ngo - edit";
		}*/
	    }
	    #ngo - author's ngo - edit (close) #modified
	    
	    #ngo - author or author's ngo - non-edit (open)
	    if(empty($user_role)){
		...
		if(!empty($user_part_or_ngo_row)){
			...
			$user_role = "ngo - author or author's ngo: reject task application mysql load";
		    }
		}elseif($task_row->task_author_id == $GLOBALS['voluno_variables']['current_user_extended']->usersext_id){
		    $user_role = "ngo - author or author's ngo";
		}
	    }
	    #ngo - author or author's ngo - non-edit (open)
	...
	#volunteer (open)
	if(empty($user_role) AND in_array('Volunteer',$function_userpositions_get['simple_pure_array']['accepted'])){
	    ...
	    $volunter_application_check_query = $GLOBALS['wpdb']->prepare('SELECT *
								...
								WHERE application_type_general = "task"
								    ...
								    AND application_status IN ("pending","accepted","rejected","dismissed","resigned");'
								,$get_id
								,$GLOBALS['voluno_variables']['current_user_extended']->usersext_id);
	    $volunter_application_check_row = $GLOBALS['wpdb']->get_row($volunter_application_check_query);
	    
	    #volunteer - not applied (open)
	    if(empty($volunter_application_check_row)){
		
		$user_role = "volunteer - not applied";
		
	    }
	    #volunteer - not applied (open)
	    
	    #volunteer - pending (open)
	    if(empty($user_role) AND $volunter_application_check_row->application_status == "pending"){
		
		$user_role = "volunteer - pending";
		
	    }
	    #volunteer - pending (open)
	    
	    #volunteer - rejected (open)
	    if(empty($user_role) AND $volunter_application_check_row->application_status == "rejected"){
		
		$user_role = "volunteer - rejected";
		
	    }
	    #volunteer - rejected (open)
	    
	    #volunteer - accepted (open)
	    if(empty($user_role) AND $volunter_application_check_row->application_status == "accepted"){
		
		$user_role = "volunteer - accepted";
	...
	if(1==1){
	    ...
	    #ngo - author - create new task (ngo_select + edit + 1) (open)
	    if($user_role == "ngo - author - create new task"){
	    ...
	    if($user_role == "ngo - author's ngo - edit"){
	    ...
	    #ngo - author or author's ngo: reject task application mysql load (open)
	    if($user_role == "ngo - author or author's ngo: reject task application mysql load"){
	    ...
	    #ngo - author or author's ngo: reject task application mysql load (close)
	...
	if(1==1){
	    ...
	    #volunteer - not applied (open)
	    if($user_role == "volunteer - not applied"){
	    ...
	    #volunteer - not applied (close)
	    
	    #volunteer - pending (open)
	    if($user_role == "volunteer - pending"){
	    ...
	    #volunteer - pending (close)
	    
	    #volunteer - rejected (open) #dome
	    if($user_role == "volunteer - rejected"){
		
		
	    }
	    #volunteer - rejected (close)
	    
	    #volunteer - accepted (open)
	    if($user_role == "volunteer - accepted"){
	    ...
	    #volunteer - accepted (close)
	    
	}
	#volunteer (close)
	
	#no access rights (open) #dome
	if(1==1){
	    
	}
	#no access rights (open)
...
#mysql (open)
if($arm['mysql'] == "yes" AND empty($reprocess_arm)){
	...
	if(1==1){
	    ...
	    if($time_expiration > strtotime($task_row->task_deadline) AND $task_row->task_deadline != "0000-00-00 00:00:00"){
	    ...
	    if($time_expiration > strtotime($task_row->task_assignment_deadline) AND $task_row->task_deadline != "0000-00-00 00:00:00"){
	...
	#update via edit mode OR create new task (open)
	if($arm['edit_mode'] == "yes" AND $_POST['activate_mysql_update_in_edit_mode'] == "yes"){
		...
		#redirect to new task (open)
		if($arm['redirect_to_new_task'] == "yes" AND $_POST['save_only_or_save_and_close_or_publish_and_save_edit_mode'] == "save_and_publish_edit_mode"){
	...
	#update via edit mode OR create new task (close)
	...
	if(1==1){
	    $progress_reports_query = $GLOBALS['wpdb']->prepare('SELECT *
							...
							ON work_tasks_rep_author = usersext_id
						     WHERE work_tasks_rep_task_id = %d
...
#style (open)
if($arm['style'] == "yes" AND empty($reprocess_arm)){
    echo '
    <style>';
...
#jquery (open)
if($arm['jquery'] == "yes" AND empty($reprocess_arm)){
...
#content (open)
if($arm['content'] == "yes" AND empty($reprocess_arm)){
	...
	if($arm['left_side_content_divs'] == "yes"){
	    ...
	    #task type (open)
	    if($arm['edit_mode'] == "yes" AND $arm['basic_info'] == "yes"){
		...
		if(1==1){
			...
			$function_div_table_selects_query2 = 'SELECT *
								FROM fi4l3fg_voluno_lists_task_types
									...
									SELECT *
									FROM fi4l3fg_voluno_items_tasks_properties
									WHERE taskprop_type = "type"
									    AND taskprop_task_id = '.$get_id.'
								    ) as aaa_selected
								    ON task_type_id = taskprop_type_id
			...
			$function_div_table_selects_name = "task_category_selection";//php_name instead of random id, also top class name. must be valid class name
			$function_div_table_selects_display_text_mysql = "task_type_name"; //what you see (mysql)
			$function_div_table_selects_value_mysql = "task_type_id"; //what php sees (mysql)
			$function_div_table_selects_input_name = "edit_task_type_id"; //$_POST['-->this<--']
			$function_div_table_selects_selected_column_name = "taskprop_type_id"; //default: "pers_settings_value"
			$function_div_table_selects_all_children = "no";
			$function_div_table_selects_cols_per_row = 3; //obligatory, regarding top level
			$function_div_table_selects_max_selected_checkboxed = $max_of_allowed_task_category_selection;
	...
	if($arm['right_side_content_div'] == "yes"){
		...
		if(1==1){
		    ...
		    if(1==1){
				...
				#function-profile-link.php (v1.0) (open)
				if(!empty($task_row->ngo_id) AND $task_row->ngo_id != 0){ //ngo
...
if($arm['task_section_2'] == "yes" AND empty($reprocess_arm)){
